...
def request(server = None, token = None, course = None, assignment = None, **kwargs):
    submission_infos = lms.api.assignment.fetchscores.request(server = server, token = token, course = course,
    ...
    temp_dir = lms.util.file.get_temp_path(prefix = 'lms-submission-files-')
...
def _write_files(temp_dir, file_contents):
    for (email, contents) in file_contents.items():
        try:
            ...
            lms.util.file.write_avoid_collisions(path, contents)
